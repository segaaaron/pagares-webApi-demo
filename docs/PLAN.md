...
**Nada nuevo mientras quede algo sin firmar** (ADR 0019). Emitir devuelve **409 `debtor_has_unsigned_note`** si el deudor tiene un pagaré en `PENDING_SIGNATURE` o `PROCESSING_SIGNATURE`, y el mensaje trae el folio que falta por firmar. No la comprueba cada caso de uso: vive dentro de **`NoteFactory`**, la única puerta por la que nace un pagaré. Emitir, renovar e importar pasan por ahí —con lo derivado, folio, token, importe en letra y prescripción, en el mismo sitio—, así que un camino nuevo la cumple sin acordarse. La importación queda exenta porque entra firmada en papel y no suma pendientes. La serie completa sí se emite —sus cuotas nacen en el mismo acto—; lo anulado y lo renovado no bloquean; y el pagaré que se renueva no cuenta contra sí mismo, porque renovar no suma un título sino que lo cambia por otro. La emisión se serializa con un cerrojo por teléfono y la comprobación busca por teléfono, no por ficha: sin eso, volver a teclear al deudor creaba una ficha nueva y la regla se saltaba sola. Las fichas **no** se unen por teléfono —dos personas que comparten línea acabarían con el pagaré de una a nombre de la otra—, así que dos deudores con el mismo número se bloquean entre sí y el 409 dice de qué folio se trata.
...
**La firma es por pagaré** (ADR 0021). Cada título se firma por separado, con su propio trazo y su propia constancia: `POST /notes/:id/signature` es la única puerta y no hay firma de serie. Un pagaré es un título ejecutivo autónomo, y lo que evita cualquier discusión ante un juez es que lleve la firma que el deudor puso en **ese** documento. Y **no se repite en ninguno**: firmar dos pagarés con la misma imagen al byte se rechaza con `409 signature_reused`, indicando el folio donde ya se usó. El coste es real —doce cuotas son doce firmas, y hasta terminarlas no se le emite nada nuevo (ADR 0019)—, así que la aplicación tiene que decir cuántas faltan y qué desbloquean, y capturar un trazo nuevo cada vez.
